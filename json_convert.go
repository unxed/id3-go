...
// v1Field strips the NUL padding of a fixed-size ID3v1 field.
func v1Field(s string) string {
	...
	if _, err := fi.Seek(0, os.SEEK_SET); err == nil {
		if v1Tag := v1.ParseTag(fi); v1Tag != nil {
			meta.ID3v1 = &V1Metadata{
				Title:   v1Field(v1Tag.Title()),
				Artist:  v1Field(v1Tag.Artist()),
				Album:   v1Field(v1Tag.Album()),
				Year:    v1Field(v1Tag.Year()),
				Comment: v1Field(v1Tag.Comments()[0]),
			}
		}
	}
